(function($) {
	...
	$.extend(SliderObject.prototype, {
		...
		create: function(a, b) {
			...
			this.pagebox = $('<div id="icon_num" class="pageBox" />')
			this.esqueleto.selector = $('<div class="' + this.options.name + '-selectors img_pagebox" />').insertAfter(a);
			this.slbtn = $('<div class="w-slbtn" />')
			this.partr = $('<div class="partr" />')
			this.partr.append(d.selectores);
			this.slbtn.append(this.partr)
			this.pagebox.append(this.slbtn)
			this.esqueleto.selector.append(this.pagebox)
			...
			if (this.options.navigation) {
                ...
                var h = $('<div><a href="#" class="' + this.options.name + '-navigation-prev" ' +
                          'rel="-1"><span>Prev</span></a><a href="#" class="' + this.options.name +
                          '-navigation-next" rel="+1"><span>Next</span></a></div>');
				this.esqueleto.navigation.append(h)
			}
		...
		addEvents: function() {
            ...
            // 点切换
			this.esqueleto.selector.find("a").mouseover(function(a) {
            ...
            // 左右箭头切换
			if (e.navigation) {
				this.esqueleto.navigation.children("div").children("a").click(function(a) {
					if (c.events.playing == false) {
						var b = c.events.stopped;
						c.stopTimer();
						c.callSlide($(this).attr("rel"));
		...
		pauseTimer: function() {
			...
			if (this.options.timer) {
				...
				var a = 100 - (parseInt(this.esqueleto.clock.bar.width(), 10) * 100 / this.options.slide_width);
		...
		callSlide: function(a) {
			...
			// 点切换中的当前点
			var c = this.esqueleto.selector.find("." + this.options.name + '-selector-current');
			var d = {};
			if (a == "+1") { // 下一个图片
				var e = b.next("." + this.options.name + '-slide');  // 下一张轮番图
				var f = c.next();  // 下一个点
				d = {
					"direction": "left"
				};
				if (e.length <= 0) {  // 当前是最后一张轮番图，则切换到第一张图片
					if (this.options.loop) {
						e = this.element.children("." + this.options.name + '-slide:first');  // 下一张: 第一张
						f = this.selectores.filter("a:first")  // 下一点: 第一个点
					} else {
						this.stopTimer();
						return false
					}
				...
				var f = this.esqueleto.selector.find("." + this.options.name + '-selector[rel=' + a + ']')
			}
		...
		transition: function(a, b, c, d, e) {
			...
			if (nxtTrans === 'random') {
				...
				f = this.transitions[0].toLowerCase()
				//}
				nxtTrans = f
			}
		...
		transbar: function(b, d, e, f, g) {
			...
			this.events.playing = true;
			var h = {
				'width': Math.round(this.options.slide_width / this.options.bars),
				'height': this.options.height
			};
		...
		transsquare: function(b, c, d, e, f) {
			...
			var g = Math.round(this.options.slide_width / this.options.columns);
		...
		transexplode: function(a, b, c, d, e) {
			...
			b.removeClass(this.options.name + '-selector-current');
			d.addClass(this.options.name + '-selector-current');
			var f = Math.round(this.options.slide_width / this.options.columns);
